    // Prepilot-2 (10 Participants on prolific)
    // No reward for one category and 20p for correct performance on high-reward category trials. 
    ...
    var phase2_timeline = [];
    var phase3_timeline = [];
    var memory_timeline = []; //only for half the participants - other half after 24 hours 
    var survey_timeline = [];
    var end_timeline = [];

    // experimental parameters 
    var TRIAL_REPETITIONS = 1; //change number of high/low dist rew when changing trial reps
    var PARTICIPANT;
    var MOD;
    var WORDS=[];
    var WORDS_SHUFFLED='';
    ...
    var Stimuli_Memory = jsPsych.randomization.repeat([].concat(stimuli_animals1,stimuli_objects1), 1);
    var Current_Phase;
    var REW_VAL;
        ...
        pages:[
         ...
         "<p>Please read the following information to decide if you are happy to participate in this study.</p>"+
          "<p><br>This study aims to investigate how people learn new words."+
            ...
            "<p>In one of the phases, you can earn rewards on a subset of trials for correct performance,"+
            "<br>and the total reward you earn will be your bonus payment.</p>"+
              ...
              "<p>The whole study will take no more than 30 minutes.</p>"+
    ...
    var instructions2 = {
        ...
        "</p><p><b>Press J</b> to begin.</p>", //add option here for more practice?
    ...
    var instructions3 = {
      type: "html-keyboard-response",
      stimulus: "<p>The next phase of learning will be the same as phase 1, but <b>some trials</b> will be rewarded with a bonus."+
      "</p><p>The more trials you match correctly, the higher your final bonus payment could be.</p>"+
      "</p><p>Phase 2 will take no more than 10 minutes."+
...
/*
    var COUNT_i=-1;
    var COUNT_j=-1;
    set_reward_dist = function(){
      var Shuffled_high = []];
      var Shuffled_low = jsPsych.randomization.repeat([10, 30, 50, 70, 90], 4);
        CURRENT_CATEGORY = jsPsych.data.getLastTrialData().values()[0].category
        if(HIGH_REWARD_CATEGORY == CURRENT_CATEGORY){ 
          COUNT_i = COUNT_i + 1;                  //pick from higher dist
          return Shuffled_high[COUNT_i];
        } else { 
          COUNT_j = COUNT_j + 1;                  //pick from lower dist 
          return Shuffled_low[COUNT_j];}}*/
    
    TOTAL_SCORE='';
    set_score = function(){
      if (TOT==0){TOTAL_SCORE = ""+roundNumber(SCORE,12)+"p";console.log(SCORE,TOT, TOTAL_SCORE);return TOTAL_SCORE}
      else if (TOT<1){TOTAL_SCORE = ""+roundNumber(SCORE,12)+"0p";console.log(SCORE,TOT, TOTAL_SCORE);return TOTAL_SCORE}
      else if (TOT==1){TOTAL_SCORE = "£1";SCORE=0;return TOTAL_SCORE} else if (TOT>1 && TOT<2){TOTAL_SCORE = 1+roundNumber(SCORE,12);console.log(SCORE, TOT, TOTAL_SCORE);return "£"+TOTAL_SCORE+""}
      else if (TOT==2){TOTAL_SCORE = "£2";SCORE=0;return TOTAL_SCORE} else if (TOT>2 && TOT<3){TOTAL_SCORE = 2+roundNumber(SCORE,12);console.log(SCORE,TOT, TOTAL_SCORE);return "£"+TOTAL_SCORE+""}
      else if (TOT==3){TOTAL_SCORE = "£3";SCORE=0;return TOTAL_SCORE} else if (TOT>3 && TOT<4){TOTAL_SCORE = 3+roundNumber(SCORE,12);console.log(SCORE, TOT, TOTAL_SCORE);return "£"+TOTAL_SCORE+""}
      else if (TOT==4){TOTAL_SCORE = "£4";SCORE=0;return TOTAL_SCORE} else if (TOT>4 && TOT<5){TOTAL_SCORE = 4+roundNumber(SCORE,12);console.log(SCORE,TOT, TOTAL_SCORE);return "£"+TOTAL_SCORE+""}
      else if (TOT==5){TOTAL_SCORE = "£5";SCORE=0;return TOTAL_SCORE} 
    }
    ...
    set_outcome = function(key_press,correct_key) {
      ...
      if(HIGH_REWARD_CATEGORY == CURRENT_CATEGORY && key_press == correct_key){
          REW_VAL = 0.20
          SCORE = SCORE+REW_VAL
          TOT = roundNumber((TOT+REW_VAL),12)
            var html = "<div style='padding-top:50px; font-size: 35px; color:"+green+";'><p><b>+0.20p</b></div>";
            html += "<div class='frame'><span class='helper'></span><img src='../img/goldstar.jpg' style='vertical-align:middle' height='240px' width='240px'></div>";
        ...
        html += "<div class='frame'><span class='helper'></span><img src='../img/sad.jpg' style='vertical-align:middle' height='190px' width='190px'></div>";
    ...
    var test_1 = {
      ...
      //timeline_variables: stimuli_4testing,     
      randomize_order: false,
    ...
    var end_info = {
      ...
      data: {trial_id: 'instructions'}};
    //<a href="https://app.prolific.co/submissions/complete?cc=2714E33E" target="_blank">Click on this link to confirm your submission to Prolific</a>
    //redirect to https://app.prolific.co/submissions/complete?cc=2714E33E
    ...
    var pilot_questions2 = {
      ...
      {prompt:"Q5. Did you notice that only one category was rewarded?",required:true,rows:1,columns:80},
    ...
    var submit_prolific = {
      ...
      "</p><p><b>Press J</b> when submission is complete.</p>", //add option here for more practice?
      trial_duration: 60000,
      choices: ['j'],
      data: {trial_id: 'end'},
    ...
    var instructions_break_2 = {
      ...
      "</p><p>Now you may take a break for a few minutes (not more than 5 minutes)."+
      "</p><p><b>Press J</b> when you are ready to continue to the next phase of learning.</p>"+
      "</p><p><br><b>Note: </b>The study will move onto the next page automatically after 5 minutes."; //add option here for more practice?
      return html},
      trial_duration: 300000,
      choices: ['j'],
      data: {trial_id: 'instructions'},
  ...
  jsPsych.init({
      ...
      //on_finish: function(){saveData("RL_1Day", jsPsych.data.get().csv());}
      }); 